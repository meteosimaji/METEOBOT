...
COOKIE_TXT_PATH = COOKIE_DIR / "cookies.txt"
...
def _cookie_to_netscape_line(cookie: dict[str, Any]) -> str:
...
def _convert_cookie_json(data: list[dict[str, Any]]) -> str:
    lines = [NETSCAPE_HEADER]
    lines.extend(_cookie_to_netscape_line(cookie) for cookie in data)
    return "\n".join(lines) + "\n"


def _load_cookie_json(path: Path) -> list[dict[str, Any]]:
...
def _configure_cookie_env() -> None:
    cookies_txt = _ensure_cookies_txt()
    if cookies_txt and not os.getenv("SAVEVIDEO_COOKIES_FILE"):
        os.environ["SAVEVIDEO_COOKIES_FILE"] = str(cookies_txt)
        log.info("Defaulted SAVEVIDEO_COOKIES_FILE to %s", cookies_txt)
