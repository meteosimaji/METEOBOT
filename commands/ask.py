...
class Ask(commands.Cog):
    ...
    async def _ask_impl(
        ...
        instructions = (
            ...
            "For /messages, you can pass keywords plus filters like from:, mentions:, has:, before:, after:, during:, before_id:, after_id:, in:, scope:, pinned:true/false, server:, or scan:; during: uses the server timezone from /settime and omitting a count defaults to 50. The display count is always capped at 50, so increase results by narrowing with before:/after:/during:/before_id:/after_id:/in:/scope:. scan: only sets a maximum scan limit (it does not increase the display count); when filters are used without scan:, it scans all available history, so scan: is optional. "
            "Filter meanings: before:/after: accept YYYY-MM-DD (server timezone) or UNIX time (seconds/ms, UTC) to include messages before/after a timestamp; during: uses a server-timezone day window; before_id:/after_id: anchor by message ID; in: limits to specific channels; scope: scans multiple channels (all/global/category). Use the current time/timezone above when deciding date filters. "
            "For in:, prefer channel mentions, IDs, or links (works across servers); channel-name lookups only resolve within the current server and may be ambiguous. "
            "For scope:, use all, global, or category=<id|name> to scan across multiple channels (use either in: or scope:). "
